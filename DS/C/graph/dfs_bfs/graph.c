...
{
	queue_node_t* q =NULL;
	q = (queue_node_t*)xmalloc(sizeof(queue_node_t));
	assert(q != NULL);
	q->pv_node = pv_node;
	q->q_next = NULL;
	q->q_prev =NULL;
...
status_t dequeue(queue_node_t* p_queue, vnode_t** pv_node)
{
	...
	*pv_node = p_queue->q_next->pv_node;//data popped up here data is node
...
{
	queue_node_t* q_run =NULL;
	queue_node_t* q_run_next =NULL;
	for(q_run =p_queue->q_next;q_run != p_queue;q_run =q_run_next)
	{
		q_run_next = q_run->q_next;
		free(q_run);
	}
	free(p_queue);//delete dummy node
	return (SUCCESS);
}
...
void bfs(graph_t* G,vertex_t S)
{
	...
	vnode_t* source_node = v_search_node(G->pv_head_node,S);
	...
	while(!is_queue_empty(queue))
	{
		...
		for(hnode_t* ph_run = pv->ph_head_node->h_next;
			ph_run != pv->ph_head_node;
			ph_run = ph_run->h_next)
		{
			vnode_t* pu =NULL;
			pu = v_search_node(G->pv_head_node,ph_run->v);
