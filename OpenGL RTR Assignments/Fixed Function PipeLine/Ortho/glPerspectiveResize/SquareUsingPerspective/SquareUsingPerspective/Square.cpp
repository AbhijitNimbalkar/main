...
void resize(int width, int height)
{
	...
	if (width <= height)
	{
		glFrustum(-0.5f, 0.5f, -0.5f*GLfloat(height) / GLfloat(width), 0.5f*GLfloat(height) / GLfloat(width), -0.5f, 0.5f);
	}
	else
	{
		glFrustum(-0.5f*GLfloat(width) / GLfloat(height), 0.5f*GLfloat(width) / GLfloat(height), -0.5f, 0.5f, -0.5f, 0.5f);
	}
}
void uninitialize()
{
